...
for (s in 1:n.scen) {
  ...
  while (t>0) {
    if ((formula.1(x.dur[t])+tolerab.func(x.dur[t]))>max.p) {
      flag=0
  ...
  for (i in 1:n.sim) {
    ...
    find.min.dur<- function (data.mfp, indices) {
      ...
      acceptability<-function(x) {
        return(y.dur.est[100]-tolerab.func(x)+0*x)
      }
    ...
    min.duration<-max.dur
    while ((t<length(poss.durations))&(flag==1)) {
      if ((up.bounds.CI[t]-tolerab.func(poss.durations[t]))<0) {
        flag=0
...
save.image("Bootstrap CI - Estimand AF.RData")
