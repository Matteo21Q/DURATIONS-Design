...
source("/lustre/scratch/scratch/rmjlmqu/Durations Inference/R Scripts/Simulation Scenarios.R")

# Set working directory
setwd("/lustre/scratch/scratch/rmjlmqu/Durations Inference/Data")
...
# Set working directory
setwd("/lustre/scratch/scratch/rmjlmqu/Durations Inference/Results")
...
for (s in 1:n.scen) {
  ...
  predicted<-function(x, fit) { 
    return(predict(object=fit,newdata=data.frame(durlong=x), type="resp"))
  }
  ...
  for (i in 1:n.sim) {
    ...
    find.min.dur<- function (data.mfp, indices) {
      ...
      fit.i<-mfp(y~fp(durlong), data=da, family = "binomial")
      
      # Predict duration-response curve and associated pointwise CI
      invisible(capture.output(y.dur.est<-predicted(x.dur, fit.i)))
      invisible(capture.output(y.durations<-predicted(all.durations[n.dur], fit.i)-predicted(poss.durations, fit.i)))
...
save.image("Bootstrap CI MFP - Estimand RD.RData")
