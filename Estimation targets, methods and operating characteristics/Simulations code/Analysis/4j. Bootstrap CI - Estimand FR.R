...
for (s in 1:n.scen) {
  ...
  for (i in 1:n.sim) {
    ...
    find.min.dur<- function (data.mfp, indices) {
      ...
      invisible(capture.output(y.durations<-predicted(poss.durations, fit.i, da)))
      
      #Define acceptability curve. Constant at 10% less than control.
      acceptability<-function(x) {
        return(max.p-0.1+0*x)
      }
...
save.image("Bootstrap CI - Estimand FR.RData")
